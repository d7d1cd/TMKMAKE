...
Static  Sym_e_t *dict_tableы DICT_TAB_SZ Е;
...
Void    setup_dict ( Void ) {
        ...
        for( cnt = 0 ; cnt < DICT_TAB_SZ ; ++cnt ) {
            pp = dict_tableы cnt Е;
            while( pp ) {
                np = pp;
                pp = pp->nxt;
                free( np );
            }
            dict_tableы cnt Е       = NULL;
        }
...
Sym_e_t *sym_in_table ( Char *sym ) {
        Sym_e_t *sp = dict_tableы hash_sym( sym ) Е;
...
#endif
#endif
        if( dict_tableы h_val Е == NULL )
            dict_tableы h_val Е     = sp;
        else {
            np = dict_tableы h_val Е;
            while( np->nxt )
                np      = np->nxt;
            np->nxt = sp;
        }
...
#endif
#endif
        if( ( pp = dict_tableы h_val Е ) == NULL )
            return( FALSE );
 
        if( strcmp( pp->sym, sym ) == 0 ) {
            dict_tableы h_val Е = pp->nxt;
            free( pp );
            return( TRUE );
        }
