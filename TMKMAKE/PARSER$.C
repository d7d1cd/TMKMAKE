...
Static
    Element_t *
    expand_elem_list(Char *txt, Int16 line)
{
  Char *np = txt;
  Char *tp;
  Int16 len;
...
Static
    Element_t *
    duplicate_elem_list(Element_t *d)
{
  Element_t *hp = NULL;
  Element_t *np = NULL;
  Element_t *ep;
...
Static
    Rules_t *
    rules_add_tail(Element_t *t, Element_t *d)
{
  Rules_t *rp;
...
Static
    Void
    rules_add_dependents(Rules_t *rp, Element_t *d)
{
...
Static
    Void
    cmd_add_tail(Cmd_t **hd, Char *cmd, Int16 line)
{
...
Static
    Element_t *
    expand_depend_dyn_name(Element_t *target,
    ...
    register_rules(Void)
{
  ...
  if (cur_wrk.t1)
  {
    ...
    while (target)
    {
      ...
      else if ((rp = in_rule_list(target->name)) == NULL)
      {
        rules_add_tail(target, tp);
      }
      else if (cur_wrk.op == OP_RULES && cur_wrk.cmd == NULL)
      {
        rules_add_dependents(rp, tp);
      }
      else if (cur_wrk.op == OP_RULES &&
               rp->op == OP_RULES && rp->cmd == NULL)
      {
        rules_add_dependents(rp, tp);
        rp->cmd = cur_wrk.cmd;
      }
      else if (cur_wrk.op == OP_MULTI_RULES &&
...
Static
    Char *
    include_keyword_found(Char *rtn_txt)
{
  Char *ep;
...
Static
    Int16
    scan_directives(Char **rd_ln, Int32 *exp_value, Int16 line)
{
  Char *tp = *rd_ln;
  Char *ep;
  Int16 rc = CD_NONE;
...
Static
    Void
    cd_push(Int16 op, Int16 line, Int16 expected,
    ...
    read_line(Int16 *line)
{
  ...
  while (TRUE)
  {
    ...
    if ((cd_op = scan_directives(&rd_ln, &exp_value, *line)) == CD_NONE)
    {
      if (cd->active)
      {
...
Static
    Int16
    check_keyword(Char *txt, Char **rtn_txt)
{
  Char *tp;
...
Void dump_rule_list(void)
{
  ...
  while (rp)
  {
    ...
    log_dbg(txtbuf);
    log_dbg("Target:");
    dump_element(rp->target);
    log_dbg("dependent:");
    dump_element(rp->dependent);
    log_dbg("commands:");
    cp = rp->cmd;
    while (cp)
    {
